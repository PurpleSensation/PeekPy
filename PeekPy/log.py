...
# ═══════════════════════════════════════════════════════════════════════════════
"""
Thread-safe singleton logging system for applications requiring shared logging
across multiple modules without explicit object passing or circular imports.

Usage:
    from PeekPy.log import get_shared_logger, configure_shared_logging
    
    # Configure once at application start
    log = configure_shared_logging(debug_level=2)
    
    # Access from anywhere
    logger = get_shared_logger()
    logger.up("Processing data")
...
class LoggerSingleton:
    """
    Thread-safe singleton logger for applications requiring shared logging.
    
    This pattern is particularly useful for:
    - Multi-module applications where passing log objects is cumbersome
    - Avoiding circular import issues with logging dependencies
    - Ensuring consistent logging behavior across an entire application
    - Maritime/scientific applications where logging coordination is critical
    """
    ...
    def set_debug_level(self, level: int) -> 'Log':
        """Set debug level and return the log instance."""
        log = self.get_log()
        log.set_debug_level(level)
        return log
    
    def set_style(self, style: str) -> 'Log':
        """Set visual style and return the log instance."""
        log = self.get_log()
        log.set_style(style)
        return log
    
    def reset_log(self) -> 'Log':
        """Reset the log instance and return it."""
        log = self.get_log()
        return log.reset()
    
    def reinitialize(self, level: int = 0, style: str = "minimal", logpath: str = "logs/log.txt") -> 'Log':
        """Reinitialize the logger with fresh settings."""
        with self._lock:
            self._log = Log(logpath=logpath)
            self._log.set_debug_level(level)
            self._log.set_style(style)
            self._log = self._log.reset()
        return self._log
...
def get_shared_logger() -> 'Log':
    """
    Get the shared Log instance from anywhere in the application.
    
    Returns:
        Log: The singleton Log instance
        
    Example:
        >>> from PeekPy.log import get_shared_logger
        >>> log = get_shared_logger()
        >>> log.up("Processing sensor data")
        >>> log.log("GPS coordinates acquired")
        >>> log.down("Complete")
    """
    return _logger_singleton.get_log()


def configure_shared_logging(level: int = 0, style: str = "minimal", logpath: str = "logs/log.txt") -> 'Log':
    """
    Configure the shared logger with specified settings.
    
    Args:
        level: Debug verbosity level (0=normal, 1=verbose, 2+=debug)
        style: Visual style ("minimal", "reinassance")  
        logpath: Path for log file output
        
    Returns:
        Log: The configured singleton Log instance
        
    Example:
        >>> from PeekPy.log import configure_shared_logging
        >>> log = configure_shared_logging(level=2, style="reinassance")
        >>> # Now all modules using get_shared_logger() will use these settings
    """
    return _logger_singleton.reinitialize(level=level, style=style, logpath=logpath)


def set_shared_debug_level(level: int) -> 'Log':
    """Set debug level on the shared logger."""
    return _logger_singleton.set_debug_level(level)


def set_shared_style(style: str) -> 'Log':
    """Set visual style on the shared logger."""
    return _logger_singleton.set_style(style)


def reset_shared_logger() -> 'Log':
    """Reset the shared logger state."""
    return _logger_singleton.reset_log()
...
def shared_log(msg: str) -> None:
    """Log a message using the shared logger."""
    get_shared_logger().log(msg)


def shared_up(msg: str) -> None:
    """Log a message and increase indentation level."""
    get_shared_logger().up(msg)


def shared_down(msg: str = "") -> None:
    """Decrease indentation level and optionally log a message."""
    if msg:
        get_shared_logger().log(msg)
    get_shared_logger().down(msg if msg else False)


def shared_warning(msg: str) -> None:
    """Log a warning using the shared logger."""
    get_shared_logger().warning(msg)